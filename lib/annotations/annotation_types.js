...
class BaseAnnotationObj {
    ...
    extract(annot_obj, page, cryptoInterface) {
        ...
        if (annot_obj["/T"])
            this.title = util_1.Util.convertUnicodeToString(cryptoInterface.decrypt(annot_obj["/T"], this.object_id));
        if (annot_obj["/TU"])
            this.alternate_description = util_1.Util.convertUnicodeToString(cryptoInterface.decrypt(annot_obj["/TU"], this.object_id));
        if (annot_obj["/DA"])
            this.default_appearance = util_1.Util.convertUnicodeToString(cryptoInterface.decrypt(annot_obj["/DA"], this.object_id));
        if (annot_obj["/M"])
